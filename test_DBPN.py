...
class Dataset_DBPN():
    def __init__(self, image_dir, patch_size, upscale_factor, data_augmentation):
...
class Dataset_DBPN_Eval():
    def __init__(self, lr_dir, upscale_factor):
...
dataset_test = Dataset_DBPN(image_dir="/home/hoo/ms_dataset/DIV2K/DIV2K_train_HR",patch_size=40,upscale_factor=4,data_augmentation=False)

dataset = ds.GeneratorDataset(dataset_test, ["input", "target","bicubic"], shuffle=True)


for data in dataset.create_dict_iterator():
    plt.imshow(data['bicubic'].asnumpy().squeeze(), cmap=plt.cm.gray)
    plt.show()
